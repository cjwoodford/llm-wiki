---
title: "The AI Doc: Or How I Became an Apocaloptimist"
type: "entity"
category: "media"
tags:
  - documentary
  - film
  - ai-ethics
  - existential-risk
  - human-flourishing
  - apocaloptimism
aliases:
  - The AI Doc: Or How I Became an Apocaloptimist
  - The AI Doc
  - How I Became an Apocaloptimist
  - Apocaloptimist
  - Apocaloptimism
---

# The AI Doc: Or How I Became an Apocaloptimist

***The AI Doc: Or How I Became an Apocaloptimist*** (2026) is a feature documentary directed by Academy Award-winning filmmaker Daniel Roher (*Navalny*) and Charlie Tyrell, produced in collaboration with the filmmakers behind *Everything Everywhere All at Once*.

The film explores the profound existential dread and utopian promise surrounding the rise of artificial general intelligence, framed through director Daniel Roher's personal journey as an expectant father trying to understand the world his child will inherit.

---

## 🎬 Synopsis & Key Themes

1. **The "Apocaloptimist" Stance**:
   - The documentary coins and defends the posture of the **Apocaloptimist**: an individual who fully acknowledges the catastrophic, potentially apocalyptic risks of unconstrained artificial intelligence (extinction, authoritarian control, democratic collapse, cultural atrophy) while simultaneously refusing fatalism and actively working toward a positive, human-centered future of flourishing and abundance.
2. **The Battle for the Future of Mind**:
   - Features wide-ranging, intimate interviews across the spectrum of the AI landscape, including frontier lab leaders ([[Sam Altman|Sam Altman]] of [[OpenAI|OpenAI]], [[Dario Amodei|Dario Amodei]] of [[Anthropic|Anthropic]], Demis Hassabis of [[Google DeepMind|Google DeepMind]]), ethical and safety critics (Tristan Harris of the [[Center for Humane Technology|Center for Humane Technology]], Yuval Noah Harari, [[Timnit Gebru|Timnit Gebru]], Eliezer Yudkowsky), and independent philosophers.
3. **Public Agency & Democratic Action**:
   - Rather than treating AI development as an inevitable technical fate decided by a handful of Silicon Valley corporations, the film serves as a populist call to action, urging citizens to assert democratic governance, demand ethical guardrails, and actively shape the technology.

---

## 💡 Core Perspectives Explored in the Film

```text
┌────────────────────────────────────────────────────────────────────────┐
│                        THE APOCALOPTIMIST SPECTRUM                     │
├──────────────────────────┬─────────────────────────────────────────────┤
│ Perspective              │ Representatives & Core Stance in the Film   │
├──────────────────────────┼─────────────────────────────────────────────┤
│ 1. Techno-Utopians       │ • Sam Altman, Dario Amodei                  │
│    (The Accelerators)    │ • Superintelligence will cure disease, solve│
│                          │   climate change, and birth cosmic wealth.  │
├──────────────────────────┼─────────────────────────────────────────────┤
│ 2. Existential Doomers   │ • Eliezer Yudkowsky, AI safety researchers  │
│    (The P-Doom Cohort)   │ • High probability of uncontrollable AGI    │
│                          │   leading to human extinction.              │
├──────────────────────────┼─────────────────────────────────────────────┤
│ 3. Societal & Epistemic  │ • Tristan Harris (CHT), Timnit Gebru        │
│    Critics               │ • Immediate harms: attention destruction,   │
│                          │   epistemic collapse, corporate monopolies. │
├──────────────────────────┼─────────────────────────────────────────────┤
│ 4. The Apocaloptimist    │ • Daniel Roher, Charlie Tyrell              │
│    (Active Synthesis)    │ • Clear-eyed dread + proactive democratic   │
│                          │   hope for [[Human Flourishing and Technology Innovation|human flourishing]].          │
└──────────────────────────┴─────────────────────────────────────────────┘
```

---

## 🔗 Related Concepts & Entities

- **Concepts**:
  - [[Human Flourishing and Technology Innovation|Human Flourishing & Technology Innovation]]
  - [[Wisdom and the Meaning Crisis|Wisdom, Mortality & The Meaning Crisis]]
  - [[TESCREAL and The Merge|TESCREAL & The Merge]]
  - [[Decoloniality and Empire Technologies|Empire Technologies & The Windigo Mind]]
  - [[Constitutional AI and Model Welfare|Constitutional AI & Model Welfare]]
- **Entities**:
  - [[Timnit Gebru|Timnit Gebru]]
  - [[Sam Altman|Sam Altman]]
  - [[Dario Amodei|Dario Amodei]]
  - [[Google DeepMind|Google DeepMind]]
  - [[OpenAI|OpenAI]]
  - [[Anthropic|Anthropic]]
  - [[Center for Humane Technology|Center for Humane Technology]]
  - [[Chad Woodford|Chad Woodford]]
